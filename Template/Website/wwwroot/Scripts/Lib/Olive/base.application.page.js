...
var BaseApplicationPage = (function () {
    ...
    BaseApplicationPage.prototype.initialize = function () {
        ...
        $("[data-control=collapsible-checkboxes]").each(function (i, e) { return _this.enableCollapsibleCheckboxes($(e)); });
        ...
        $("[data-change-action][data-control=collapsible-checkboxes]").off("ccl.change.data-action").off("change.data-action").on("ccl.change.data-action", function (e) { return _this.invokeActionWithAjax(e, $(e.currentTarget).attr("data-change-action")); });
    ...
    BaseApplicationPage.prototype.enableCollapsibleCheckboxes = function (input) {
    ...
    BaseApplicationPage.prototype.replaceListControlSource = function (controlId, items) {
        ...
        if ($control.data('control').toString() === 'collapsible-checkboxes') {
            var $itemsContainer = $control.find('.options-container .items-list .checkbox-list');
            $itemsContainer.empty();
            for (var i = 0; i < items.length; i++) {
                $itemsContainer.append($("<div><input type='checkbox' id='" + controlId + "_" + i + "' name='" + controlId + "' value='" +
                    items[i].Value + "'><label for='" + controlId + "_" + i + "'>" + items[i].Text + "</label></div>"));
            }
            this.enableCollapsibleCheckboxes($control); // re-initialize the collapsible checkbox control
        }
