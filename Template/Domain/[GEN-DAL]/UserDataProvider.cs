namespace AppData
{
    ...
    public class UserDataProvider : SqlDataProvider<Domain.User>
    {
        #region SQL Commands
        ...
        internal void FillData(IDataReader reader, Domain.User entity, UserFields fields)
        {
        ...
        async Task Insert(Domain.User item)
        {
        ...
        async Task Update(Domain.User item)
        {
            if ((await ExecuteScalar(UPDATE_COMMAND, CommandType.Text, CreateUserParameters(item))).ToStringOrEmpty().IsEmpty())
            {
        ...
        protected IDataParameter[] CreateUserParameters(Domain.User item)
        {
