...
namespace Modules
{
    public class LoginForm : FormModule<Domain.User>
    {
        public LoginForm()
        {
            ...
            Property<bool>("MustShowCaptcha");
            
            Property("Microsoft.AspNetCore.Mvc.ViewFeatures.ViewDataDictionary", "CaptchaSettings").Getter(@"new ViewDataDictionary(new EmptyModelMetadataProvider(), new ModelStateDictionary())
            {
                {""RefreshButtonText"", ""Refresh the captcha image""},
                {""RequiredMessage"", ""Enter the code shown in the image""}
                }");
                
                Property<bool>("IsCaptchaValid");
                
                Property<bool>("SuccessfulLogin").Getter(@"if (MustShowCaptcha && !IsCaptchaValid) return false;
                
                return !InvalidCredentials && !Item.IsDeactivated;");
                
                Property<bool>("InvalidCredentials").Getter("Item == null || !SecurePassword.Verify(Password, Item.Password, Item.Salt)");
                ...
                Link("Forgot password?").Icon(FA.Key)
                    .Action(x => x.Go<Login.ForgotPasswordPage>());
